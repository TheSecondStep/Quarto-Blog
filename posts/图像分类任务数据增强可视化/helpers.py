...
def show_one(img, pixels_per_inch=100, max_inches=(8, 8)):
    img = F.to_image(img)
    img = F.to_dtype(img, torch.uint8, scale=True)

    h, w = img.shape[-2], img.shape[-1]
    fig_w = w / pixels_per_inch
    fig_h = h / pixels_per_inch

    # 可选：避免图太大把窗口撑爆
    fig_w = min(fig_w, max_inches[0])
    fig_h = min(fig_h, max_inches[1])
        ...
        for col_idx, img in enumerate(row):
            ...
            if boxes is not None:
                img = draw_bounding_boxes(
                    img, boxes, colors="yellow", width=bbox_width)
            if masks is not None:
                img = draw_segmentation_masks(img, masks.to(torch.bool), colors=[
                                              "green"] * masks.shape[0], alpha=.65)
